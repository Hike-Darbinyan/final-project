...
const TaskGroup: React.FC = ({}) => {
  ...
  const onDragEnd = (result: any) => {
    ...
    const sourceColumn: TaskProps = tasks.find(
      (item) => item.id.toString() === source.droppableId
    ) as TaskProps;
    const destinationColumn: TaskProps = tasks.find(
      (item) => item.id.toString() === destination.droppableId
    ) as TaskProps;

    const newSourceCards: TaskItemProps[] = Array.from(
      sourceColumn?.tasks as TaskItemProps[]
    );
    ...
    if (source.droppableId === destination.droppableId) {
      ...
      const newDestinationCards: TaskItemProps[] = Array.from(
